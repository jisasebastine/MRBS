...
namespace youbefit.Controllers
{
    ...
    public class RoomController: Controller
    {
        ...
        public IActionResult AddRoom([FromBody] RoomDto room)
        {
            ...
            if (new_room == null)
                return Ok(new { message = "The meeting room is alreay added" });
            else return Ok(new_room);
        }
        ...
        public IActionResult GetBooking([FromBody] RoomDto room)
        {
            ...
            if (bookings == null)
                return Ok(new { message = "No bookings yet" });
        ...
        public IActionResult BookRoom([FromBody] RoomDto room)
        {
            ...
            if (new_room == null)
                return Ok(new { message = "Room already booked during that time" });
            return Ok(new
            {
                MeetingRoomId = new_room.MeetingRoomId,
                MeetingRoomName = new_room.MeetingRoomName,
                NextBooking = new_room.NextBooking,
                Vacant = new_room.Vacant
            }
            );
        }
